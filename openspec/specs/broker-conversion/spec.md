...
- **AND** extract Symbol and Date (lapse date) from transaction row
...
- **THEN** key the FMV by (Symbol, Date) from the transaction row

#### Scenario: RSU vesting with awards file

- **WHEN** transactions CSV contains `Stock Plan Activity` for symbol on date
- **AND** awards file contains FMV price for that symbol and date (within 7-day lookback)
...
#### Scenario: FMV 7-day lookback

- **WHEN** exact date match not found in awards file
