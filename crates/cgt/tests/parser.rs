...
fn test_parse_valid_buy() {
    ...
    let transactions = parse(input).expect("Failed to parse valid BUY transaction");
...
fn test_parse_dividend_with_tax_keyword() {
    ...
    let transactions = parse(input).expect("Failed to parse DIVIDEND with TAX keyword");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
    assert_eq!(
        tx.date,
...
fn test_parse_capreturn_with_fees_keyword() {
    ...
    let transactions = parse(input).expect("Failed to parse CAPRETURN with FEES keyword");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
    assert_eq!(
        tx.date,
...
fn test_parse_split_with_ratio_keyword() {
    ...
    let transactions = parse(input).expect("Failed to parse SPLIT with RATIO keyword");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
    assert_eq!(
        tx.date,
...
fn test_parse_unsplit_with_ratio_keyword() {
    ...
    let transactions = parse(input).expect("Failed to parse UNSPLIT with RATIO keyword");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
    assert_eq!(
        tx.date,
...
fn test_parse_buy_without_currency_defaults_to_gbp() {
    ...
    let transactions = parse(input).expect("Failed to parse BUY without currency");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
...
fn test_parse_buy_with_gbp_currency_treated_as_default() {
    ...
    let transactions = parse(input).expect("Failed to parse BUY with explicit GBP");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
...
fn test_parse_split_not_confused_with_currency() {
    ...
    let transactions = parse(input).expect("Failed to parse SPLIT");
    assert_eq!(transactions.len(), 1);
...
fn test_parse_fees_keyword_not_confused_with_currency() {
    ...
    let transactions = parse(input).expect("Failed to parse");
...
fn test_parse_dividend_without_tax_clause() {
    ...
    let transactions = parse(input).expect("Failed to parse DIVIDEND without TAX clause");
    assert_eq!(transactions.len(), 1);
    let tx = &transactions[0];
    assert_eq!(
        tx.date,
...
fn test_parse_dividend_without_tax_clause_with_currency() {
    ...
    let transactions =
        parse(input).expect("Failed to parse DIVIDEND without TAX clause (with currency)");
    assert_eq!(transactions.len(), 1);
    if let Operation::Dividend {
        total_value,
        tax_paid,
...
fn test_parse_sell_without_fees_clause() {
    ...
    let transactions = parse(input).expect("Failed to parse SELL without FEES clause");
    assert_eq!(transactions.len(), 1);
...
fn test_parse_capreturn_without_fees_clause() {
    ...
    let transactions = parse(input).expect("Failed to parse CAPRETURN without FEES clause");
    assert_eq!(transactions.len(), 1);
