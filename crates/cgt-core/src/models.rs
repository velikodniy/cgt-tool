...
/// Validates that the year is in the range 1900-2100.
...
impl TaxPeriod {
    ...
    ///
    /// Returns `None` only if the year is outside chrono's valid range,
    /// which should never happen for validated TaxPeriod values.
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if parts.len() != 2 {
            return Err(serde::de::Error::custom(format!(
                "invalid tax period format: expected 'YYYY/YY', got '{}'",
                s
            )));
        }
        ...
        if end_short != expected_end {
            return Err(serde::de::Error::custom(format!(
                "tax years must be consecutive: '{}' should end with '{:02}', not '{:02}'",
                s, expected_end, end_short
            )));
        }
...
///
/// For B&B matches, `acquisition_date` contains the date of the matched purchase.
/// For SameDay and Section104 matches, `acquisition_date` is None.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, JsonSchema)]
pub struct Match {
    pub rule: MatchRule,
    pub quantity: Decimal,
...
///
/// Contains one or more tax year summaries and end-state holdings.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, JsonSchema)]
